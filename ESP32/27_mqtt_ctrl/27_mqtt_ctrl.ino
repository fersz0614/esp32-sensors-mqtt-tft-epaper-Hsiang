...
constexpr char MQTT_CTRL_TOPIC[] = "Hsiang/class305/ctrl";
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strstr(message, "\"gled\"") != nullptr) {
    name = "GLED";
    ledPin = LED_GREEN;
  } else if (strstr(message, "\"yled\"") != nullptr) {
    name = "YLED";
    ledPin = LED_YELLOW;
  } else if (strstr(message, "\"rled\"") != nullptr) {
    name = "RLED";
    ledPin = LED_RED;
  } else {
    return;
  ...
  bool on = strstr(message, "\"on\"") != nullptr;
  bool off = strstr(message, "\"off\"") != nullptr;
  if (!on && !off) return;
...
bool connectMqtt() {
  ...
  if (ok) mqttClient.subscribe(MQTT_CTRL_TOPIC);
