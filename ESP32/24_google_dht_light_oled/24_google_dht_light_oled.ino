...
constexpr char SHEET_ID[] = "1xPUW_jhTFVC8TJthiWnef2BqVDeWKtv-WPamYopdars";
constexpr char SHEET_NAME[] = "data1";
constexpr char SCRIPT_URL[] =
    "https://script.google.com/macros/s/AKfycbyR-Yp-uu4nIvnjvnkILaQ5AX8yFxp-UpBO-Sqs0su3ai1N_BvQsz_Q/exec";
...
void sendToGoogleSheets() {
  ...
  String data = String(temperature) + "," + String(humidity) + "," +
                String(lightPercent);
  String url = String(SCRIPT_URL) + "?type=insert&dateInclude=1&sheetId=" +
               urlEncode(SHEET_ID) + "&sheetTag=" + urlEncode(SHEET_NAME) +
               "&data=" + urlEncode(data);
  ...
  if (http.begin(client, url)) {
    ...
    Serial.printf("Google Sheet HTTP %d, response: %s\n", httpCode,
                  response.c_str());
    uploadStatus = (httpCode >= 200 && httpCode < 400) ? "Uploaded OK"
                                                       : "Upload error";
    http.end();
  } else {
